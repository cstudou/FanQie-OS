...
static void memory_pool_init(uint32_t memory_size)
{
    ...
    kernel_pool.memory_pool_bitmap.bits = (void *)(0xc009a000); //位图的地址
...
//将物理内存回收到内存池
void page_free(uint32_t page_physical_addr)
{
    struct Pool *pool;
    uint32_t index = 0;
    if(page_physical_addr >= user_pool.physical_addr_start)
    {
        pool = &user_pool;      //用户内存池
        index = (page_physical_addr - user_pool.physical_addr_start) / 4096;  //第几个页面
    }
    else
    {
        pool = &kernel_pool;
        index = (page_physical_addr - kernel_pool.physical_addr_start) / 4096;
    }
    bitmap_set_musk(&(pool->memory_pool_bitmap), index, 0);
}

void page_pte_remove(uint32_t vaddr)
{
    uint32_t *pte = pte_ptr(vaddr);
    uint32_t temp = 1;
    *pte &= ~(temp);
    asm volatile("invlpg %0"::"m"(vaddr):"memory");     //更新tlb,传进去的必须是实际的内存地址，用内存约束m
}

//释放从vaddr开始的size个虚拟页
void vaddr_remove(enum MemoryPoolFlag flag, void *vaddr, uint32_t size)
{
    uint32_t index = 0, cnt = 0;
    if(flag == KERNELPOOL)
    {
        //内核内存池
        index = ((uint32_t)vaddr - kernel_virtual_addr.virtual_addr_start) / 4096;
        while(cnt < size)
        {
            bitmap_set_musk(&kernel_virtual_addr.virtual_addr_bitmap, index+cnt, 0);
            cnt++;
        }
    }
    else
    {
        struct TaskStruct *thread = running_thread();
        index = ((uint32_t)vaddr - thread->user_addr.virtual_addr_start) / 4096;
        while(cnt < size)
        {
            bitmap_set_musk(&thread->user_addr.virtual_addr_bitmap, index+cnt, 0);
            cnt++;
        }
    }
...
//释放从vaddr开始的size个物理页
void paddr_remove(enum MemoryPoolFlag flag, void *addr, uint32_t size)
{
    uint32_t index = 0, cnt = 0;
    uint32_t vaddr = (uint32_t)addr;
    uint32_t paddr = addr_v2p(vaddr);       //得到物理地址
    if(paddr >= user_pool.physical_addr_start)
    {
        while(cnt < size)
        {
            paddr = addr_v2p(vaddr);
            page_free(paddr);           //清空物理地址
            page_pte_remove(vaddr);     //删除pte
            vaddr += 4096;
            cnt ++;
        }
        vaddr_remove(flag, addr, size); //清除虚拟地址
    ...
    else
    {
        while(cnt < size)
        {
            paddr = addr_v2p(vaddr);
            page_free(paddr);
            page_pte_remove(vaddr);
            vaddr += 4096;
            cnt ++;
        }
        vaddr_remove(flag, addr, size);
    }
...
void sys_free(void *ptr)
{
    ...
    struct Arena *area = block2area(ptr);          //转换成arena

    if(area->large && area->desc == NULL)
    {
        paddr_remove(flag, area, area->cnt);
    }
    else
    {
        list_push_back(&(area->desc->free_list), &(block->memory_list));
        area->cnt ++;
        if(area->cnt == area->desc->arena_nums)         //所有内存块都空闲
        {
            int idx = 0;
            for(; idx<area->desc->arena_nums; idx++)
            {
                struct MemoryBlock *temp = area2block(area, idx);
                list_remove(&temp->memory_list);        //删除所有结点
            }
            paddr_remove(flag, area, 1);
        }
...
//没有虚拟地址池位图操作
void* get_page_fork(enum MemoryPoolFlag flag, uint32_t vaddr)
{
    struct Pool* memory_pool = flag & KERNELPOOL ? &kernel_pool : &user_pool;       //申请区域是用户内存还是内核内存
    lock_acquire(&memory_pool->lock);
    void *page = palloc(memory_pool);
