...
{
    return (struct Arena *)((uint32_t)block & 0xfffff0000);
}
...
void *sys_malloc(uint32_t size)
{
    ...
    {
        //内核线程
        enum MemoryPoolFlag flag = KERNELPOOL;
        uint32_t pool_size = kernel_pool.memory_pool_size;
        struct Pool *memory_pool = &kernel_pool;
        struct MemoryDesc *desc = kernel_memory_desc;
    }
    else
    {
        enum MemoryPoolFlag flag = USERPOOL;
        uint32_t pool_size = user_pool.memory_pool_size;
        struct Pool *memory_pool = &user_pool;
        struct MemoryDesc *desc = thread->user_memory_desc;
    }
