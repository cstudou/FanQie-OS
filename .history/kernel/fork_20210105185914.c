...
int32_t build_child_stack(struct TaskStruct *child)
{
    ...
    uint32_t* ret_addr_in_thread_stack  = (uint32_t*)stack - 1;

   /***   这三行不是必要的,只是为了梳理thread_stack中的关系 ***/
   uint32_t* esi_ptr_in_thread_stack = (uint32_t*)stack - 2; 
   uint32_t* edi_ptr_in_thread_stack = (uint32_t*)stack - 3; 
   uint32_t* ebx_ptr_in_thread_stack = (uint32_t*)stack - 4; 
   /**********************************************************/

   /* ebp在thread_stack中的地址便是当时的esp(0级栈的栈顶),
   即esp为"(uint32_t*)intr_0_stack - 5" */
   uint32_t* ebp_ptr_in_thread_stack = (uint32_t*)stack - 5; 
   ...
   *edi_ptr_in_thread_stack = *esi_ptr_in_thread_stack = 0;
   ...
   child->self_stack = ebp_ptr_in_thread_stack;	    
