...
#define IDT_DESC_CNT 0x20      // 目前总共支持的中断数
...
/* 创建中断门描述符 */
static void make_idt_desc(struct gate_desc *interrupt_gate, uint8_t attritube, void* function)
{
    interrupt_gate->func_offset_low_word = (uint32_t)function & 0x0000ffff;
    interrupt_gate->func_offset_high_word = ((uint32_t)function >> 16) & 0x0000ffff;
    interrupt_gate->dcount = 0;
    interrupt_gate->attribute = attritube;
    interrupt_gate->selector = SELECTOR_CODE;
}
