...
static void make_idt_desc(struct InterruptGate *interrupt_gate, uint8_t attritube, void* function)
{
...
static void make_idt(struct InterruptGate* p_gdesc, uint8_t attritube, void* function) { 
   p_gdesc->func_offset_low = (uint32_t)function & 0x0000FFFF;
   p_gdesc->selector = SELECTOR_CODE;
   p_gdesc->fixed = 0;
   p_gdesc->attribute = attritube;
   p_gdesc->func_offset_high = ((uint32_t)function & 0xFFFF0000) >> 16;
}
