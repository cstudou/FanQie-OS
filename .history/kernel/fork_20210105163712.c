...
/* b 复制父进程的虚拟地址池的位图 */
   uint32_t bitmap_pg_cnt = ((0xc0000000 - USER_ADDR_START) / 4096 / 8 );
   ...
   memcpy(vaddr_btmp, child_thread->user_addr.virtual_addr_bitmap.bits, bitmap_pg_cnt * 4096);
   child_thread->user_addr.virtual_addr_bitmap.bits = vaddr_btmp;
   /* 调试用 */
