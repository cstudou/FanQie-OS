...
void select_disk(struct Disk *disk)
{
   ...
   if (disk->device_no == 1) {	// 若是从盘就置DEV位为1
...
void intr_hd_handler(uint8_t irq_no) {
   ...
   struct Channel* channel = &channels[ch_no];
  // ASSERT(channel->irq_no == irq_no);
/* 不必担心此中断是否对应的是这一次的expecting_intr,
 * 每次读写硬盘时会申请锁,从而保证了同步一致性 */
Puts("this is intr\n");
   if (channel->in_intr) {
      channel->in_intr = false;
...
void ide_init()
{
   ...
   printk("\n   all partition info\n");
   /* 打印所有分区信息 */
   list_traversal(&partition_list, info, (int)NULL);
