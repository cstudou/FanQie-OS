...
static void ticks_to_sleep(uint32_t sleep_ticks) {
   uint32_t start_tick = kernel_clock;

   while (kernel_clock - start_tick < sleep_ticks) {	   // 若间隔的ticks数不够便让出cpu
      thread_yield();
   }
}
void ticks_sleep(uint32_t ticks)
{
    /*
    //中断评率是100，1个中断10ms
    uint32_t sleep_ticks = ticks / 10;
    uint32_t start_ticks = kernel_clock;
    //kernel_clock记录开机以来总ticks
    while(kernel_clock - start_ticks < sleep_ticks)
    {
        thread_out();
    }
    */
   uint32_t sleep_ticks = DIV_ROUND_UP(ticks, mil_seconds_per_intr);
