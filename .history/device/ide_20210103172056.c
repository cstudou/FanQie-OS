...
void intr_disk_handler(uint8_t intr_no)
{
...
void partitioan_scan(struct Disk *disk, uint32_t lba)
{
    ...
    while(index++ < 4)
    {
        ...
        else if (table->fs_type != 0)
        {
            ...
            {
                //Puts("this is main\n");
                disk->logic_parts[l_no].start_lba = lba_start + table->start_lba;
                disk->logic_parts[l_no].section_num = table->sec_cnt;
                disk->logic_parts[l_no].disk = disk;
                Puts("-------\n");
            Putint((uint32_t)list_size(&partition_list));
            Puts("--------\n");
                list_push_back(&partition_list, &disk->logic_parts[l_no].part_tag);
...
bool info(struct ListPtr *list, int arg )
{
    ...
    Puts("      "); Putint((uint32_t)part); Puts(part->name); Puts(" start_lba:0x");
...
void ide_init()
{
    ...
    /* 打印所有分区信息 */
    Puts("---------\n");
    Putint(list_size(&partition_list));
    Puts("---------\n");
