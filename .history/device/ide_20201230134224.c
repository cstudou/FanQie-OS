...
void swap_bytes(const char *x, const char *buf, uint32_t len)
{
    uint8_t index;
...
void identify_disk(struct Disk *disk)
{
    ...
    char temp[64];
    uint8_t sn_start = 10 * 2, sn_len = 20, md_start = 27 * 2, md_len = 40; swap_bytes
...
{
    struct BootSector* boot_sector = sys_malloc(512);
...
bool info(struct ListPtr *list, int arg UNUSED)
{
    struct Partition pa;
    uint32_t len =  (uint32)&pa.part_tag - (uint32)&pa;
