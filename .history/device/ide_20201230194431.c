...
static bool busy_wait(struct disk* hd) {
   ...
   while (time_limit -= 10 >= 0) {
      if (!(readb(reg_status(channel)) & BIT_STAT_BSY)) {
	 return (readb(reg_status(channel)) & BIT_STAT_DRQ);
      } else {
	 ticks_sleep(10);		     // 睡眠10毫秒
      }
   }
   return false;
}
...
static void partition_scan(struct disk* hd, uint32_t ext_lba) {
   struct boot_sector boo;
   struct boot_sector* bs = &boo;//sys_malloc(sizeof(struct boot_sector));
   Puts("begin");
...
/* 打印分区信息 */
static bool partition_info(struct ListPtr* pelem, int arg __attribute__ ((unused))) {
    struct partition pa;
    uint32_t len =  (uint32_t)&pa.part_tag - (uint32_t)&pa;
    struct partition *part = (struct partition *)((uint32_t)pelem - len);
