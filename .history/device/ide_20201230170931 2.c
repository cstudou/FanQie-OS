...
/* 打印分区信息 */
static bool partition_info(struct ListPtr* pelem, int arg) {
    struct partition temp;
    uint32_t len = (uint32_t)((uint32_t)&temp.part_tag - (uint32_t)&temp);
   struct partition* part = (struct partition*)(pelem - len);
